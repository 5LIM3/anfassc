    ...
    <section ref={ref} style={{ background: "#003d24", padding: "6rem 3rem", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <div className="ms-fade" style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "1rem" }}>
          ...
          <span style={{ fontWeight: 600, fontSize: "11px", letterSpacing: "4px", textTransform: "uppercase", color: "#D4AF37" }}>Our Mission</span>
        </div>
        <h2 className="ms-fade" style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, color: "#fff", lineHeight: 1.15, marginBottom: "4rem", maxWidth: "500px" }}>
          Built on Passion.<br />Driven by Purpose.
        </h2>

        <div className="rgrid-3" style={{ gap: "2px" }}>
          {MISSIONS.map((m) => (
            <div key={m.num} className="ms-fade" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", padding: "2.5rem 2rem", cursor: "default" }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "3rem", fontWeight: 900, color: "rgba(212,175,55,0.25)", lineHeight: 1, marginBottom: "1rem" }}>{m.num}</div>
              <div style={{ fontWeight: 700, fontSize: "1.1rem", letterSpacing: "1px", textTransform: "uppercase", color: "#D4AF37", marginBottom: "0.75rem" }}>{m.title}</div>
              <p style={{ fontSize: "0.95rem", color: "rgba(255,255,255,0.6)", lineHeight: 1.7, margin: 0 }}>{m.body}</p>
            </div>
          ))}
        </div>
      </div>
      <style>{`.ms-fade { opacity: 0; transform: translateY(24px); transition: opacity 0.6s ease, transform 0.6s ease; } .ms-fade.ms-vis { opacity: 1; transform: translateY(0); }`}</style>
    </section>
  );
}